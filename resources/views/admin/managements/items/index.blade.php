...
@section('content')
    ...
    <div class="bg-white dark:bg-slate-700/50 rounded-l-xl dark:border-l dark:border-slate-700 shadow-xl px-6 py-4 flex flex-col h-[calc(100vh)]"
        x-data="{
            ...
            reopenFailedModal() {
                @if ($errors->has('discount_percentage') && old('item_slug')) let failedItem = {{ Js::from($items) }}.find(item => item.slug === '{{ old('item_slug') }}');
                    if (failedItem) {
        ...
        <div x-show="updateDiscountModal" x-cloak x-transition.opacity
            class="fixed inset-0 bg-black/40 backdrop-blur-sm z-50">
        </div>
        <div x-show="updateDiscountModal" x-cloak x-transition
            class="fixed inset-0 z-50 flex items-center justify-center p-4">
                ...
                <form :action="`/managements/items/${itemToUpdateDiscount.slug}/discount`" method="POST" class="mt-4">
                    ...
                    <div>
                        ...
                        <input type="number" name="discount_percentage" id="discount_percentage"
                            value="{{ old('discount_percentage') }}" :value="itemToUpdateDiscount.discount_percentage"
