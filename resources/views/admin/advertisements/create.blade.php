...
@section('title', 'Tambah Iklan Baru')
@section('content')
    <div class="bg-white dark:bg-slate-700/50 rounded-l-xl dark:border-l dark:border-slate-700 shadow-xl px-6 py-4 flex flex-col h-[calc(100vh)] overflow-y-auto">
        ...
        <div class="border-b border-gray-200 dark:border-slate-600/50 mb-6 pb-4" data-aos="fade-up">
            ...
            <h2 class="text-2xl font-bold text-gray-700 dark:text-white">Buat Iklan Baru</h2>
            <p class="text-sm text-gray-500 dark:text-slate-400 mt-1">Isi formulir di bawah ini untuk membuat draf artikel baru.</p>
        </div>

        {{-- Form input for create advertisments --}}
        <form action="{{ route('advertisements.store') }}" method="POST" enctype="multipart/form-data" class="flex flex-col flex-grow" data-aos="fade-up">
            @csrf
            <div class="flex-grow overflow-y-auto pr-4 space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="target_url" id="target_url" value="{{ old('target_url') }}" class="w-full bg-gray-100 dark:bg-slate-700 border border-gray-300 dark:border-slate-600 text-gray-800 dark:text-gray-200 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Contoh: https://namadomain.com...">
