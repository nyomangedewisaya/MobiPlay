...
@section('content')
    ...
    <div class="bg-white dark:bg-slate-700/50 rounded-l-xl dark:border-l dark:border-slate-700 shadow-xl px-6 py-4 flex flex-col h-[calc(100vh)]"
        x-data="{
            ...
            openAdvertisementModal(product) {
                this.advertisementToShow = product;
        ...
        <div
            ...
            <div class="flex items-center gap-2 w-full sm:w-auto">
                <form action="{{ route('advertisements.index') }}" method="GET" class="relative w-full sm:w-auto">
                    ...
                    <button type="button" x-show="searchQuery"
                        @click="searchQuery = ''; window.location.href = '{{ route('advertisements.index') }}'" x-cloak
        ...
        <div class="flex-grow flex flex-col overflow-hidden" data-aos="fade-up">
            ...
            <div class="flex-shrink-0">
                <h3 class="text-lg font-semibold text-gray-700 dark:text-slate-200">Dipublish (<span
                        class="text-green-500">{{ count($activeAdvertisements) }}</span>)</h3>
            ...
            <div class="overflow-x-auto pb-4 -ml-4 sm:-ml-6 pl-4 sm:pl-6">
                <div class="grid grid-flow-col grid-rows-2 auto-cols-[15rem] sm:auto-cols-[16rem] gap-4 lg:flex lg:gap-4">
                    @forelse ($activeAdvertisements as $advertisement)
                        <div @click="openAdvertisementModal({{ json_encode($advertisement) }})"
                            ...
                            <img src="{{ $advertisement->banner_url }}" alt="{{ $advertisement->title }}"
                                class="w-full h-32 object-cover rounded-t-lg">
                            <div class="p-3">
                                <h4 class="font-bold text-gray-800 dark:text-white truncate">{{ $advertisement->title }}</h4>
                            </div>
                        </div>
                    @empty
            ...
            {{-- Inactive advertisements (drafts) --}}
            <div class="flex-shrink-0 mt-4">
                <h3 class="text-lg font-semibold text-gray-700 dark:text-slate-200">Draf (<span
                        class="text-yellow-500">{{ count($inactiveAdvertisements) }}</span>)</h3>
                <hr class="dark:border-slate-600 my-2">
            </div>
            <div class="overflow-x-auto pb-4 -ml-4 sm:-ml-6 pl-4 sm:pl-6">
                <div class="grid grid-flow-col grid-rows-2 auto-cols-[15rem] sm:auto-cols-[16rem] gap-4 lg:flex lg:gap-4">
                    @forelse ($inactiveAdvertisements as $advertisement)
                        <div @click="openAdvertisementModal({{ json_encode($advertisement) }})"
                            class="lg:w-70 bg-gray-50 dark:bg-slate-800 rounded-lg shadow-md cursor-pointer hover:scale-101 hover:shadow-lg transition-all duration-300">
                            <img src="{{ $advertisement->banner_url }}" alt="{{ $advertisement->title }}"
        ...
        {{-- Modal detail advertisement --}}
        <div x-show="advertisementModal" x-cloak x-transition.opacity class="fixed inset-0 bg-black/40 backdrop-blur-sm z-50">
        </div>
        <div x-show="advertisementModal" x-cloak x-transition class="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div x-cloak x-transition.opacity
                class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                <div @click.outside="advertisementModal = false"
                    ...
                    <div class="p-4 border-b dark:border-slate-700 flex justify-between items-center">
                        <h3 class="text-xl font-bold text-gray-800 dark:text-white" x-text="advertisementToShow.title"></h3>
                        <button @click="advertisementModal = false"
                    ...
                    <div class="p-6 space-y-4 overflow-y-auto">
                        <img :src="advertisementToShow.banner_url" :alt="advertisementToShow.title"
                            class="w-full rounded-lg aspect-video object-cover bg-gray-200 dark:bg-slate-700">
                        <p class="text-gray-600 dark:text-slate-300 whitespace-pre-wrap" x-text="advertisementToShow.target_url"></p>
                    </div>
                    <div class="p-4 border-t dark:border-slate-700 flex flex-wrap justify-end gap-4">
                        <form :action="`/advertisements/${advertisementToShow.slug}`" method="POST">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
                                class="px-4 py-2 rounded-lg red-gradient-wh text-white font-medium">Hapus</button>
                        </form>
                        <a :href="`/advertisements/${advertisementToShow.slug}/edit`"
                            class="px-4 py-2 rounded-lg blue-gradient-wh text-white font-medium">Edit</a>
                        <form :action="`/advertisements/${advertisementToShow.slug}/status`" method="POST">
                            @csrf
                            @method('PATCH')
                            <template x-if="advertisementToShow.status === 'active'">
                                <button type="submit"
                                    class="px-4 py-2 rounded-lg yellow-gradient-wh text-white font-medium">Jadikan
                                    Private</button>
                            </template>
                            <template x-if="advertisementToShow.status === 'inactive'">
                                <button type="submit"
